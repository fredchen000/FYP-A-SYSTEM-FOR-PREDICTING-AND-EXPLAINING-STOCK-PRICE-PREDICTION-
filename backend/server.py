...
avail_symbols= ["AAPL", "AMZN", "GOOG", "MSFT", "JPM"]
...
def predict():
    ...
    stock_with_absolute = pd.read_pickle('./'+symbol+'/data/stock_with_absolute.pkl')
    label_abs_1d = pd.read_pickle('./'+symbol+'/data/label_abs_1d.pkl')
    X_train, X_test, y_train, y_test = train_test_split(stock_with_absolute, label_abs_1d, test_size=0.1, shuffle=True)
    scaler = MinMaxScaler()
    ...
    news_list, google_url = get_news(symbol, target.index[0].strftime("%m/%d/%Y"))
    ...
    for news in news_list:
        ...
        title_sentiments.append(blob.sentiment.polarity)
        des_sentiments.append(des_blob.sentiment.polarity)
    ...
    return_dict["title_sent"] = title_sentiments
    return_dict["desc_sent"] = des_sentiments
    
    target = scaler.transform(target)
    return_dict["news"] = news_list
    return_dict["news_url"] = google_url
    return_dict["LGR"] = [int(pickle.load(open('./'+symbol+'/LR/LR_1d.pkl','rb')).predict(target)[0]),
                          int(pickle.load(open('./'+symbol+'/LR/LR_7d.pkl','rb')).predict(target)[0]),
                          int(pickle.load(open('./'+symbol+'/LR/LR_30d.pkl','rb')).predict(target)[0])]
    return_dict["DT"] = [int(pickle.load(open('./'+symbol+'/DT/DT_1d.pkl','rb')).predict(target)[0]),
                          int(pickle.load(open('./'+symbol+'/DT/DT_7d.pkl','rb')).predict(target)[0]),
                          int(pickle.load(open('./'+symbol+'/DT/DT_30d.pkl','rb')).predict(target)[0])]
    return_dict["RF"] = [int(pickle.load(open('./'+symbol+'/RF/RFC_1d.pkl','rb')).predict(target)[0]),
                          int(pickle.load(open('./'+symbol+'/RF/RFC_7d.pkl','rb')).predict(target)[0]),
                          int(pickle.load(open('./'+symbol+'/RF/RFC_30d.pkl','rb')).predict(target)[0])]
    return_dict["XGB"] = [int(pickle.load(open('./'+symbol+'/XGB/XGB_1d.pkl','rb')).predict(target)[0]),
                          int(pickle.load(open('./'+symbol+'/XGB/XGB_7d.pkl','rb')).predict(target)[0]),
                          int(pickle.load(open('./'+symbol+'/XGB/XGB_30d.pkl','rb')).predict(target)[0])]
